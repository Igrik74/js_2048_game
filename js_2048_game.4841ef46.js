...
})({"5T6O9":[function(require,module,exports,__globalThis) {
...
},{"../modules/Game.class.js":"dhlFI","@parcel/transformer-js/src/esmodule-helpers.js":"lBAIH"}],"dhlFI":[function(require,module,exports,__globalThis) {
'use strict';
class Game {
    constructor(initialState){
        ...
        this.score = 0;
        this.status = 'idle';
        if (initialState && initialState.length === this.size) this.state = initialState.map((row)=>[
                ...row
            ]);
        else this.state = this.createEmptyBoard();
        this.initialBoard = this.state.map((row)=>[
                ...row
            ]);
    }
    createEmptyBoard() {
    ...
    start() {
        ...
        this.status = 'playing';
        this.state = this.initialBoard.map((row)=>[
                ...row
            ]);
        this.score = 0;
        this.spawnNewCell();
        ...
        switch(direction){
            case 'left':
                this.state = this.state.map((row)=>{
                    ...
                    if (!moved && !this.rowsEqual(row, newRow)) moved = true;
                    return newRow;
                });
                break;
            case 'right':
    ...
    processRowRight(row) {
        ...
        for(let i = nums.length - 1; i > 0; i--)if (nums[i] === nums[i - 1]) {
            nums[i] *= 2;
            this.score += nums[i];
            nums[i - 1] = 0;
            i--;
        ...
        while(nums.length < this.size)nums.unshift(0);
        return nums;
    }
    processRowLeft(row) {
        const reversed = [
    ...
    transpose(matrix) {
        ...
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++)result[i][j] = matrix[j][i];
        return result;
    }
    updateStatus() {
        // win
        ...
        for (const row of this.state)for (const cell of row){
            if (cell === 0) return;
        }
        // check horizontal moves
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size - 1; j++){
            if (this.state[i][j] === this.state[i][j + 1]) return;
        }
        // check vertical moves
        for(let i = 0; i < this.size - 1; i++)for(let j = 0; j < this.size; j++){
            if (this.state[i][j] === this.state[i + 1][j]) return;
        }
        this.status = 'lose';
    }
}
...
},{}],"lBAIH":[function(require,module,exports,__globalThis) {
...
},{}]},["5T6O9","3Aj1C"], "3Aj1C", "parcelRequire7b01", {})
